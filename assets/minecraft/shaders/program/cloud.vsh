...
void main()
{
    ...
    vec3 sunPosition2 = -sunPosition3 * clamp(skyIntensityNight, 0, 1);
    ...
    for (int i = 0; i < maxIT; i++)
    {
        ...
        samplee = 2.2 * max(vec3(0.0), generate(pos.xyz, sunPosition3).xyz * 2.0) / maxIT;
    ...
    */   
     lightCol.xyz = (generate(sunPosition3.xyz, sunPosition3).xyz);
	...
	vec3 sunColorCloud =  (generate(modSunVec.xyz, sunPosition3).xyz);

	//Fake bounced sunlight
	vec3 bouncedSun = lightSourceColor*1.0/5.0*0.5*clamp(lightDir*sunVec.y,0.0,1.0)*clamp(lightDir*sunVec.y,0.0,1.0);
