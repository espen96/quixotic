...
#define FLICKER_REDUCTION 0.25  //[0.0 0.25 0.5 0.75 1.0] High values reduce flickering but may reduce sharpness
    ...
    float depth = texture(CurrentFrameDepthSampler, texCoord).r ;
{
	...
	vec3 fragposition = toScreenSpace(position);
	fragposition = mat3(gbufferModelViewInverse) * fragposition + gbufferModelViewInverse[3].xyz + (currChunkOffset - prevChunkOffset);
	vec3 previousPosition = mat3(gbufferPreviousModelView) * fragposition + gbufferPreviousModelView[3].xyz;
	previousPosition = toClipSpace3Prev(previousPosition);
	vec2 velocity = previousPosition.xy - position.xy;
	reprojectedPosition.xy = texCoord + velocity;
	...
	float movementRejection = (0.12+isclamped)*clamp(length(velocity/oneTexel),0.0,1.0);
    ...
    vec3 supersampled =  invTonemap(mix(tonemap(clippedPrevColor),tonemap(currColor),clamp(BLEND_FACTOR*lumDiff2+rej+isclamped*ANTI_GHOSTING+0.01,0.0,1.0)));
